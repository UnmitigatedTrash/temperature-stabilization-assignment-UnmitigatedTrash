...
int main(void) {
    ...
    // Array that stores current and previous temperatures from clients
    float temperature[numExternals];
    float previousTemps[numExternals];

    // Stores temporary values to prevent garbage being used
    ...
    int stable = false;
    while(!stable) {
        // Store previous external temps
        for (int i = 0; i < numExternals; i++) {
            previousTemps[i] = temperature[i];
        }
        ...
        for(int i = 0;  i < numExternals; i++) {
            ...
            printf("Current Temperature of External Process (%d) = %f\n", i, temperature[i]);
        }

        // Check stability condition
        for(int i = 0; i < numExternals; i++) {
            if(fabs(temperature[i] - previousTemps[i]) < 1e-3) {
                stable = true;
                break;
            }
        }
        ...
        float avgExternalTemp = 0.0;
        for(int i = 0; i < numExternals; i++) {
            avgExternalTemp += temperature[i];
        }

        // Calculate updated temperature
        updatedTemp = ((2 * previousTemp) + avgExternalTemp) / (numExternals + 2);
    ...
    // Construct message with done signal
    struct msg stop_msg;
    stop_msg.T = -9999.0;
    stop_msg.Index = -1;

    // Send done signal to the 4 external processes
    for(int i = 0; i < numExternals; i++) {
        send(client_socket[i], (const void *)&stop_msg, sizeof(stop_msg), 0);
    }
